...
Start documentation work from `docs/README.md`. Read the canonical authority stack and only the subsystem documents needed for the active work. Historical material is available through Git history only and must not be treated as current implementation direction unless the active milestone explicitly calls for prior-behavior review.
