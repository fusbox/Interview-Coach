...
import {
    createAppAuthQueryClientFromEnv,
    type AppAuthQueryClient,
} from "./app-auth-postgres-runtime";
...
export class PostgresAppAuthStore implements AppAuthStore {
    ...
    async isCandidateAccountEligible(userId: string): Promise<boolean> {
        const result = await this.client.query(
            `
                select 1
                from public.app_user_roles app_role
                join public.candidate_profiles profile
                  on profile.app_user_id = app_role.user_id
                 and profile.workspace = 'interview_coach'
                 and profile.status = 'active'
                where app_role.user_id = $1
...
function mapUser(row: Record<string, unknown>): AppUser {
    return {
        ...
        emailVerifiedAt: toIsoString(row.email_verified_at) ?? undefined,
