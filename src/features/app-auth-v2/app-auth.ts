...
export type PasswordAuthenticationPolicy = {
    requiredRole?: AppRole;
    requireVerifiedEmail?: boolean;
    requireCandidateProfile?: boolean;
};
...
export async function authenticateWithPassword(
    ...
    policy: PasswordAuthenticationPolicy = {},
): Promise<LoginResult> {
    ...
    const policyReason = await getPolicyFailureReason(credential.user, store, policy);
    if (policyReason) {
        await recordAuditSafely(store, {
            userId: credential.user.id,
            eventType: "login",
...
async function getPolicyFailureReason(
    user: AppUser,
    store: AppAuthStore,
    policy: PasswordAuthenticationPolicy,
): Promise<string | null> {
    if (policy.requiredRole && !user.roles.includes(policy.requiredRole)) {
        return "wrong_audience";
    }
    if (policy.requireVerifiedEmail && !user.emailVerifiedAt) {
        return "email_unverified";
    }
    if (policy.requireCandidateProfile && !await store.isCandidateAccountEligible(user.id)) {
        return "candidate_profile_unavailable";
    }
    return null;
}
