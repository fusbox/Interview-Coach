...
export type CandidateVoiceTranscriptionQueryClient = {
    query: (sql: string, values: unknown[]) => Promise<{ rows: Array<Record<string, unknown>> }>;
};

export function createCandidateVoiceTranscriptionRepository(client: CandidateVoiceTranscriptionQueryClient) {
    return {
        async recoverRun(input: {
            ...
            const result = await client.query(`
                ...
                join lateral (
                  ...
                  from public.candidate_voice_transcription_runs candidate_run
                  where candidate_run.candidate_practice_session_id = session.candidate_practice_session_id
                    and candidate_run.candidate_profile_id = session.candidate_profile_id
                    and candidate_run.question_slot_id = $3
                    and candidate_run.question_index = $4::integer
                    and candidate_run.idempotency_key_hash = $5
                  order by candidate_run.generation_attempt desc
                  limit 1
                ) run on true
        ...
        async createRequestedRun(input: {
            candidateVoiceTranscriptionRunId: string;
            candidatePracticeSessionId: string;
            candidateProfileId: string;
            questionSlotId: string;
            questionIndex: number;
            ...
            const result = await client.query(`
                with owned_slot as materialized (
                  ...
                  from public.candidate_practice_sessions session
                  where session.candidate_practice_session_id = $2
                    and session.candidate_profile_id = $3
                    and session.status in ('planned', 'in_progress')
                    and exists (
                      select 1
                      from jsonb_array_elements(
            ...
            `, [
                input.candidateVoiceTranscriptionRunId,
                input.candidatePracticeSessionId,
                input.candidateProfileId,
                input.questionSlotId,
                input.questionIndex,
            ...
            return normalizeCandidateRun(result.rows[0]);
        },
                ...
                try {
                    const result = await client.query(`
                        with owned_slot as materialized (
                          ...
                          update public.candidate_voice_transcription_runs run
                          set lifecycle_state = 'failed',
                              error_code = 'STALE_TRANSCRIPTION_CLAIM',
                              completed_at = $16
                          from claim_lock
                          where run.candidate_practice_session_id = $2
                            and run.candidate_profile_id = $3
                          ...
                          returning run.candidate_voice_transcription_run_id
                          ...
                          where run.candidate_practice_session_id = $2
                            ...
                            and run.question_index = $5::integer
                            and run.idempotency_key_hash = $6
                            and run.candidate_voice_transcription_run_id not in (
                              select candidate_voice_transcription_run_id from expired_requested
                            )
                        ...
                        select case
                                 ...
                                 when owned_slot.voice_transcript_drafts_json -> $4 ->> 'sourceTranscriptionRunId'
                                      = active.candidate_voice_transcription_run_id::text then 'replayed'
                                 else 'superseded'
                               end as claim_outcome,
                               active.*,
        ...
        async findOwnedRun(input: {
            ...
            const result = await client.query(`
                select *
                from public.candidate_voice_transcription_runs
                where candidate_voice_transcription_run_id = $1
                  and candidate_practice_session_id = $2
        ...
        async completeRunAndSaveDraft(input: {
            ...
            const draft = createVoiceTranscriptDraft({
                ...
                sourceTranscriptionRunId: input.candidateVoiceTranscriptionRunId,
            ...
            const result = await client.query(`
                with owned_session as materialized (
                  ...
                  where candidate_practice_session_id = $2
                    and candidate_profile_id = $3
                  ...
                  set lifecycle_state = 'completed',
                      ...
                      completed_at = $7
                  from owned_session
                  where run.candidate_voice_transcription_run_id = $1
                    and run.candidate_practice_session_id = $2
                    and run.candidate_profile_id = $3
                    and run.question_slot_id = $4
...
function normalizeClaim(row: Record<string, unknown> | undefined): VoiceTranscriptionClaimResult | null {
    if (!row) return null;
    const run = normalizeCandidateRun(row);
...
function normalizeCandidateRun(row: Record<string, unknown> | undefined) {
    if (!row) return null;
    return normalizeVoiceTranscriptionRunRecord({
        ...row,
        voice_transcription_run_id: row.candidate_voice_transcription_run_id,
        practice_session_id: row.candidate_practice_session_id,
        audience_owner_id: row.candidate_profile_id,
    });
}
