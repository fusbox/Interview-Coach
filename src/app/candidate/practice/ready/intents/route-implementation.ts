...
function createDefaultCandidatePracticeIntentCreateDependencies(): CandidatePracticeIntentCreateDependencies {
    ...
    return {
        ...
        async createPracticeIntentFromPointers({ candidateProfileId, source, pointers, idempotencyKeyHash }) {
            const sourceSessions = await loadCandidatePracticeIntentSourceSessions({
                candidateProfileId,
                pointers,
                findSetupSession: practiceSessionRepository.findSetupSession,
            });
            const practicePlanBaselines = await practicePlanBaselineRepository.listForCandidate({ candidateProfileId });
            const practiceSessions = practicePlanBaselines.reduce((sessions, baseline) => (
                createCandidateBaselineAwarePracticeSessions({ practiceSessions: sessions, baseline })
            ), sourceSessions);
