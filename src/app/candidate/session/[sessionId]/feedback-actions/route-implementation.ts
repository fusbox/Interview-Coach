...
> {
    ...
    return {
        resolveCandidateSessionIdentity: async (request) => {
            const devIdentity = resolveCandidateFeedbackActionIdentityFromDevLaunchCookie(request.headers.get("Cookie"));
...
async function resolveCandidateSessionIdentityFromLaunchCookie(
    request: Request,
    client: CandidateFeedbackActionQueryClient,
...
export function resolveCandidateFeedbackActionIdentityFromDevLaunchCookie(cookieHeader: string | null) {
