...
The category system answers a different question than the question-preparedness result:

- Question preparedness answers: "What evidence did this answer demonstrate, and what band did it earn?"
...
- [Question Preparedness Progress Contract](./question-preparedness-progress-contract.md)

## Product Rule

Question categories are not performance ratings. They identify the interview-demand axis and select the category-specific evidence lenses used during evaluation.
...
- category coverage comes from planned and practiced canonical questions;
- achievement comes from the question-preparedness result, not a category score;
- unanswered questions remain neutral and do not lower achievement;
...
- Describe how they have used, or would apply, relevant tools, processes, or knowledge.
- Connect knowledge to practical work.
- Explain judgment, verification, escalation, or learning when the answer depends on employer-specific or safety-sensitive procedure.
...
- Resume content may invite a relevant example but is not evidence that the candidate is qualified or that a claim in the answer is correct.
- Feedback can encourage the candidate to connect answer evidence to role-specific requirements.

Coach evidence:

- Does the answer show role-relevant knowledge?
- Does it explain practical application?
- Does it explain reasoning, verification, escalation, or limits when useful?
- When a trusted technical reference is supplied, does the answer agree with it?
...
- The current deterministic baseline reserves role-specific practice slots by interview stage. Role-aware omission or substitution remains a future planner enhancement.
- Category feedback should distinguish role knowledge from communication quality.
- Role-skill evidence must come from the answer, not inferred qualification from the JD or resume.
- Without an accepted technical reference, feedback may discuss role relevance, practical application, reasoning, and verification awareness but may not claim factual correctness.

## Category Signals And Question Preparedness

Each category selects additional evidence markers that help the evaluator interpret the five universal criteria. Category signals may be supported by an exact span, whole-answer meaning, or meaningful absence. They are not separately rated, averaged, or exposed as scores.

Application code derives one question-preparedness result from the universal criterion appraisals. The dashboard may pair that result with the question category so candidates can distinguish:

- coverage: which interview demands have been planned or practiced;
- achievement: the highest earned qualitative band for each canonical question;
- latest feedback: what the most recent accepted attempt showed and what to try next.

The same evidence may support transcript-canvas annotations and candidate-safe explanations, but the category does not create a second preparedness truth.
...
- Technical / Role-Specific: prefer role/JD-specific experience, application, process, judgment, verification, and learning questions. Avoid exact rules, numerical standards, legal/regulatory requirements, clinical guidance, hazardous-work procedures, or other authoritative factual recall when no trusted reference is supplied.
...
- Do not turn role relevance into a correctness claim or factual correction when technical accuracy is `not_assessed`.

## Dashboard Implications

- Present plan coverage separately from question achievement.
- Keep unanswered questions neutral.
- Use category labels to explain the interview demand, not to imply a hiring judgment.
- Use candidate-safe evidence and Coach Update language to explain an earned band.
- Keep numeric transformations internal and do not recreate the retired score-era lane/matrix model.
- Defer category-specific cross-session recommendation policy until the dashboard product contract deliberately defines it.
