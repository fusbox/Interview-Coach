...
| [ ] | Run full product smoke locally. | In progress: local Postgres smoke DB, schema, rollback smoke, app-user provisioning, HTTP auth/protected-page smoke, repeatable Level 2 product smoke, repeatable Level 3 AI/QA smoke, and repeatable Level 4 real SMTP invite/debrief smoke passed. Covered: invite creation, invite email send, candidate token page/session access, initials, practice start, draft save, answer submit, answer feedback, question generation, hints, strong response, session debrief, debrief email send, `ai_generations`, QA page, QA export, and related Postgres metrics. Remaining smoke: visible browser UX, profile/settings, recruiter review, resend/retry, practice-again, and negative permission checks. |
