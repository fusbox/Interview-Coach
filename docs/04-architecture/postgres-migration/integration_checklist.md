...
| [ ] | Run full product smoke locally. | In progress: local Postgres smoke DB, schema, rollback smoke, app-user provisioning, HTTP auth/protected-page smoke, repeatable Level 2 product smoke, and repeatable Level 3 AI/QA smoke passed. Covered: invite creation, candidate token page/session access, initials, practice start, draft save, answer submit, answer feedback, question generation, hints, strong response, session debrief, `ai_generations`, QA page, and QA export. Remaining smoke: visible browser UX, profile/settings, SMTP invite/debrief send, recruiter review, resend/retry, practice-again, and negative permission checks. |
