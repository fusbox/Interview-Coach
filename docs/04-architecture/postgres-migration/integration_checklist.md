...
| [ ] | Middleware/session refresh | Current `src/middleware.ts` still delegates to the migration middleware module. Supabase mode refreshes Supabase cookies; app-auth mode now performs lightweight protected-page cookie checks and redirects missing-session recruiter/admin/QA page requests to login. | Finish retiring Supabase SSR middleware after app auth is the only runtime. Authoritative DB-backed auth remains in layouts/route handlers; middleware is a lightweight browser guard. | | [Your response here]
...
| Status | To confirm | Current app | Post-migration clarification | Fu's notes | Integration team response |
| --- | --- | --- | --- | --- | --- |
| [ ] | Recruiter auth smoke | Local Postgres smoke user `fu@rangam.com` can sign in through `/api/auth/login` and reach `/recruiter` with a Postgres app-auth session. | Product/browser smoke should still confirm the visible login page and recruiter create/profile UX. |
...
| [ ] | AI surfaces | Current app calls Gemini for question generation, answer feedback, hints, strong response, and session debrief; AI-quality instrumentation records `ai_generations`. | Route-stack smoke now triggers all five surfaces with Gemini and confirms Postgres `ai_generations` plus QA read/export. Remaining validation is visible browser UX and target environment provider behavior. |
...
| [x] | Recruiter session review | Done locally for current product contract: owning recruiter can open `/recruiter/sessions/[id]` and view session summary, question text, submitted state, and transcript. The page does not expose candidate AI feedback. Cross-user/admin permission behavior remains part of the negative permission checks slice. |
...
| [x] | Invite send/resend/retry flows | Done locally: invite send/debrief send passed in Level 4 email smoke, and resend plus failed-batch retry passed in Level 9 resend/retry smoke. Target-environment validation still needs the final public host, SMTP policy, and company DB. |
...
| [ ] | Replace direct Supabase query calls in routes/pages/actions. | In progress: direct API-route `auth.getUser()` checks for question generation, invite create/send/resend/retry, ops metrics, and recruiter profile now go through `getAuthenticatedRouteUser()`. Recruiter dashboard and admin feedback profile/timezone reads now go through the app-auth-compatible profile helper. App-auth middleware now handles missing-session page redirects without importing Supabase or Postgres auth internals. Remaining work includes server components/actions, Supabase fallback repositories, auth callback, and final package/env cleanup. |

### Phase 3 - App-Owned Recruiter Auth

| Status | Work item | Notes / done when |
| --- | --- | --- |
| [ ] | Define account provisioning policy. | In progress: public self-signup remains paused, and an operator/developer provisioner now seeds phase-1 app users directly in Postgres. Final policy still needs confirmation: admin-created users, integration-seeded users, or future self-signup. |
...
| [ ] | Implement login route/server action. | In progress: `/api/auth/login` verifies credentials, creates a hashed server-side session, records auth audit events, and sets the HTTP-only app session cookie. App users can now be provisioned for smoke tests, and the local Level 2 product-flow smoke logs in successfully against Postgres app auth. Still needs route-level rate limiting and account lockout behavior updates. |
...
| [x] | Replace browser Supabase auth clients. | Done for current runtime browser clients: login posts to `/api/auth/login`, desktop/mobile logout post to `/api/auth/logout`, and settings/create/ProfileGuard read/write profile state through `/api/recruiter/profile`. Public self-signup remains paused; operator/developer provisioning is documented separately. |
| [ ] | Preserve E2E test auth seam. | Keep local E2E mode useful without live auth credentials. |
| [ ] | Add auth tests. | In progress: focused tests cover scrypt password hashing/verification, successful/failed password login orchestration, hashed session lookup/revocation, audit writes, DB-role RBAC, login cookie setting, logout cookie clearing, and app-auth middleware redirects/allows. A route-stack product-flow smoke now covers successful Postgres login. Still need password reset, lockout/rate-limit, and browser UI tests. |

### Phase 4 - Candidate Flow Preservation

| Status | Work item | Notes / done when |
| --- | --- | --- |
| [ ] | Preserve `/s/[token]` invite entry. | In progress: Level 2 product-flow smoke opens the Postgres-backed candidate link and fetches the session with `x-candidate-token`. Still needs visible browser UX and negative authorization checks. |
| [ ] | Preserve initials/intake/resume behavior. | In progress: Level 2 product-flow smoke verifies initials submission and resume text persistence through invite-created session intake. Still needs visible browser UX coverage. |
| [ ] | Preserve answer submission and recovery behavior. | In progress: Level 2 product-flow smoke verifies draft save, answer submit, answer feedback persistence, and transition to `REVIEWING`. Include the session-recovery patch decision before final migration validation. |
...
| [ ] | Run full product smoke locally. | In progress: local Postgres smoke DB, schema, rollback smoke, app-user provisioning, HTTP auth/protected-page smoke, repeatable Level 2 product smoke, repeatable Level 3 AI/QA smoke, repeatable Level 4 real SMTP invite/debrief smoke, Level 5 browser-visible recruiter invite smoke, Level 6 browser-visible candidate practice smoke, Level 7 practice-again chain smoke, Level 8 profile/settings smoke, Level 9 invite resend/retry smoke, and Level 10 recruiter review smoke passed. Covered: invite creation, visible recruiter login/create/preview/send/dashboard, invite email send, invite resend, failed-batch retry, candidate token page/session access, initials, welcome/readiness, hints, strong response, text-mode answers, answer feedback, session debrief, no-SMTP summary copy, debrief email send, chained practice-again attempts, profile/settings load/save, recruiter-owned session review, `ai_generations`, QA page, QA export, and related Postgres metrics. Remaining smoke: negative permission checks. |
...
- Continue local browser-visible smoke from [local_postgres_smoke.md](./local_postgres_smoke.md), with backend selectors pinned to `postgres` against the disposable Docker DB.
