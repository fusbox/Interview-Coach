...
| [ ] | Postgres connection format | `pg` is already installed, but current runtime code does not yet use `DATABASE_URL` or `POSTGRES_*` for app persistence. | Working approach: support both formats and normalize internally, preferring `DATABASE_URL` when present. | Proceed with flexible config until final env contract is answered. | [Your response here]
...
| [ ] | Feedback repository | `SupabaseFeedbackRepository` persists app feedback and exposes the admin feedback view by joining sessions. | Postgres implementation exists behind `FEEDBACK_REPOSITORY_BACKEND=postgres`; it preserves candidate/recruiter feedback capture, session/type update behavior, recruiter-only feedback inserts, and admin read shape with session role/intake context. Admin route auth still depends on Supabase until app auth lands. | Implemented locally and validated with focused unit/UI tests plus disposable Docker Postgres integration test. | [Your response here]
| [ ] | AI-quality repository | AI-quality capture writes and reads `ai_generations` through service-role Supabase access. | Implement Postgres-backed write/read/export/summary queries for `/qa/ai-quality`. | | [Your response here]
...
| [ ] | Idempotency store | `src/lib/server/idempotency.ts` now uses a backend seam. Supabase remains the default during migration; `IDEMPOTENCY_BACKEND=postgres` selects the Postgres store. | Postgres implementation preserves reserve, pending duplicate, conflict, complete/replay, and pending release behavior against `api_idempotency_keys`. Route-level flip remains blocked by auth/rate-limit cutover and broader route validation. | Implemented locally and validated with focused unit tests plus disposable Docker Postgres integration test. | [Your response here]
| [ ] | Candidate token store | Candidate links now use a backend seam. Supabase remains the default during migration; `CANDIDATE_TOKEN_BACKEND=postgres` selects the Postgres store. | Postgres backend preserves random raw token issuance, hash-at-rest storage, session-bound validation, and active-token checks for `revoked_at`/`expires_at`. Route-level flip still depends on surrounding session repository/auth migration. | Implemented locally and validated with focused unit tests plus disposable Docker Postgres integration test. | [Your response here]

## Authentication, Authorization, And Identity

| Status | To confirm | Current app | Post-migration clarification | Fu's notes | Integration team response |
| --- | --- | --- | --- | --- | --- |
| [ ] | Recruiter login method | `/login` uses Supabase browser auth for email/password sign-in and sign-up. | Replace with app-owned email/password auth backed by Postgres. Decide whether phase 1 allows self-sign-up or uses admin-provisioned users only. | Proceeding with email/password, not magic link. | [Your response here]
...
| [ ] | Current-user helper | `getCachedUser()` returns a Supabase `User` or an E2E test user. Many server routes/components depend on it. | Create provider-neutral app user type and helper with email, subject/id, roles/groups, and display metadata. | | [Your response here]
| [ ] | Browser-side Supabase usage | Login, logout, mobile dock, settings, recruiter create page, and profile guard use browser Supabase clients. | Replace browser auth calls with provider SDK, server actions, or app API routes. | | [Your response here]
...
| [ ] | Recruiter profile/settings | Current profile guard reads/writes `recruiter_profiles`. | User can create/update recruiter profile and see settings persist after reload/sign-out/sign-in. |
...
| [ ] | Email send | Current email uses SMTP environment vars after SES SMTP switchover. | Send invite email successfully from target deployment and confirm link host is correct. |
...
| [ ] | Resend/retry flows | Current invite send/resend/retry routes depend on auth, rate limiting, idempotency, and invite batch persistence. | Exercise send, resend, retry failure path if available, and verify durable records. |
...
10. After migration/auth code is ready, should the integration team validate unit/integration behavior in the company deployment environment before QA performs product testing?
...
| [x] | Inspect the Azure `feature/postgres-integration` branch. | Initial branch setup confirmed. Head is `5668696 bypass replay tour gate for 1 user`; detailed migration-code inventory is still covered by the Supabase touchpoint inventory item below. |
| [x] | Inventory every Supabase touchpoint. | Done: first-pass implementation inventory created at [supabase_touchpoint_inventory.md](./supabase_touchpoint_inventory.md). Revisit after the AI-quality workstream is merged because this Azure branch predates that instrumentation. |
...
| [ ] | Port SQL functions/procedures. | In progress: initial schema draft ports invite batch creation, engagement increment, rate-limit consumption, metrics counter/timing rollups, and SLO summary functions. Repository implementation may replace some DB functions with app transactions if cleaner. |
...
| [ ] | Implement Postgres AI-quality repositories. | Preserve capture writes, read filters, pagination counts, grouped views, summary widgets, and JSON/CSV export. |
...
| [ ] | Replace direct Supabase query calls in routes/pages/actions. | No `.from(...)`, `.rpc(...)`, `.auth.getUser()`, `createClient()`, or `createAdminClient()` should remain in runtime paths. |
