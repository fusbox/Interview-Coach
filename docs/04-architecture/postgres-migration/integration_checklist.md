...
| [ ] | Run full product smoke locally. | In progress: local Postgres smoke DB, schema, rollback smoke, app-user provisioning, HTTP auth/protected-page smoke, and repeatable Level 2 route-stack product smoke passed. The product smoke covers invite creation, candidate token page/session access, initials, practice start, draft save, answer submit, answer-feedback persistence, and QA-source `ai_generations` persistence through mock fallback. Remaining smoke: visible browser UX, profile/settings, SMTP invite/debrief send, hints, strong response, full-session debrief, recruiter review, and QA explorer UI record validation. |
...
- Run local product smoke levels 1-3 from [local_postgres_smoke.md](./local_postgres_smoke.md), with backend selectors pinned to `postgres` against the disposable Docker DB.
