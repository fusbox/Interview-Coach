...
| [ ] | Browser-side Supabase usage | Login, desktop logout, and mobile dock logout now use app API routes. Settings, recruiter create page, and profile guard still contain browser Supabase clients. ProfileGuard is skipped when `APP_AUTH_BACKEND=postgres`. | Replace remaining browser profile/current-user calls with app API routes or server-provided state. | | [Your response here]
...
| [x] | Inventory every Supabase touchpoint. | Done: implementation inventory created at [supabase_touchpoint_inventory.md](./supabase_touchpoint_inventory.md) and refreshed after porting AI-quality generation capture. |
...
| [ ] | Implement logout. | In progress: `/api/auth/logout` revokes the server-side session by hashed token and clears the cookie. Desktop sidebar and mobile dock now call it. |
...
| [ ] | Replace browser Supabase auth clients. | In progress: login, desktop logout, and mobile dock logout are moved to app API routes. Settings, recruiter create page, and ProfileGuard still need replacement; ProfileGuard is bypassed in app-auth mode for now. |
