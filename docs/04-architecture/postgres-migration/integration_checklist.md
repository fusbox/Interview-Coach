...
| [ ] | Add auth tests. | In progress: focused tests cover scrypt password hashing/verification, successful/failed password login orchestration, hashed session lookup/revocation, audit writes, DB-role RBAC, login cookie setting, logout cookie clearing, and app-auth middleware redirects/allows. Still need password reset, lockout/rate-limit, browser UI, and product smoke tests. |
...
| [ ] | Run full product smoke locally. | In progress: local Postgres smoke DB, schema, rollback smoke, app-user provisioning, and HTTP auth/protected-page smoke passed. Remaining product smoke: visible login/browser UX, profile/settings, invite creation, email-send decision, candidate practice, hints, strong response, answer feedback, session debrief, recruiter review, and QA explorer record validation. |
