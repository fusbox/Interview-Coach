...
| `src/lib/server/auth/app-auth.test.ts`, `src/lib/auth/rbac.test.ts`, `src/app/api/auth/*/route.test.ts` | New app-auth foundation and route tests. | Preserve and expand as middleware and remaining browser UI are wired. | Covers password hashing, password login orchestration, session token hashing, session revocation, audit writes, app-role RBAC, login cookie setting, and logout cookie clearing. | In progress |
