...
| `src/lib/server/auth/app-auth.ts`, `app-session.ts`, `password.ts`, `postgres-app-auth-store.ts` | New app-owned auth foundation replacing Supabase Auth concepts. | Uses scrypt password hashes, opaque hashed app-session tokens, `app_users`, `app_user_credentials`, `app_sessions`, `app_user_roles`, and `auth_audit_events`. | Login/logout routes, middleware, and browser UI are next; this foundation is not yet active in runtime auth flows. | In progress |
...
| `src/lib/server/auth/app-auth.test.ts`, `src/lib/auth/rbac.test.ts` | New app-auth foundation tests. | Preserve and expand as login/logout routes, middleware, and browser UI are wired. | Covers password hashing, password login orchestration, session token hashing, session revocation, audit writes, and app-role RBAC. | In progress |
