...
- Branch head during inventory: `5668696 bypass replay tour gate for 1 user`
- Date: 2026-05-04

Important branch note: this Azure branch predates the newer AI-quality generation capture work from the separate `ai-quality-generation-capture` worktree. When the workstreams are merged, add the `ai_generations` schema/repositories/QA explorer paths to this inventory before final Supabase removal.
...
| --- | --- | --- | --- | --- |
| Supabase client wrappers | `src/lib/supabase/server.ts`, `src/lib/supabase/middleware.ts` own current server/browser auth session behavior. | Replace with app auth/session helpers and Postgres pool. | High | Open |
| Recruiter auth | Supabase email/password, signup, auth callback, SSR cookies. | App-owned email/password auth, secure HTTP-only session cookie, DB-backed roles. | High | Open |
| Candidate access | Candidate token data is stored in Supabase, but candidate auth model is app-token based. | Port token storage/validation to Postgres and keep UX unchanged. | Medium | Open |
...
| Operational stores | Rate limits, metrics, idempotency, candidate tokens use service-role Supabase access and RPCs/tables. | Port tables/functions/backends to Postgres. | High | Open |
...
| Browser Supabase usage | Login/logout/settings/create/profile guard/mobile dock use `createBrowserClient`. | Replace with server actions/API routes and app session endpoints. | High | Open |
...
| --- | --- | --- | --- | --- |
| `src/lib/supabase/server.ts` | Creates user-scoped Supabase SSR client, service-role client, and `getCachedUser()` via `supabase.auth.getUser()`. Requires `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`. | Replace with app auth module: `getCurrentUser()`, `requireUser()`, `getDb()`, and separate privileged server repository access through Postgres pool. | Central seam. Keep E2E test mode behavior when replacing `getCachedUser()`. | Open |
| `src/lib/supabase/middleware.ts` | Refreshes Supabase cookies and calls `supabase.auth.getUser()` when `sb-` cookies exist. | Replace with app session cookie validation and protected-route redirect behavior. | Must preserve no-auth candidate/public route behavior. | Open |
| `src/middleware.ts` | Delegates all middleware behavior to Supabase middleware. | Delegate to new app session middleware. | Keep static asset exclusions. | Open |
| `src/app/login/page.tsx` | Browser Supabase client calls `signInWithPassword()` and `signUp()`. | Replace with app-owned login/signup form posting to server action/API route. | Decide self-sign-up vs admin-provisioned users. If self-sign-up remains, add email verification. | Open |
...
| `src/components/auth/LogoutButton.tsx` | Browser Supabase `signOut()`. | Call app logout endpoint/server action that invalidates session row and clears cookie. | Ensure desktop logout works. | Open |
| `src/components/layout/RecruiterMobileDock.tsx` | Browser Supabase client signs out; imports Supabase `User` type. | Use app user type and shared logout command. | Mobile path can otherwise remain visually unchanged. | Open |
...
| `src/components/auth/ProfileGuard.tsx` | Browser Supabase `getUser()` and profile lookup from `recruiter_profiles`. | Replace with app session/profile endpoint or server-provided profile state. | Guard should not make browser-direct DB calls. | Open |
...
| `src/app/(recruiter)/recruiter/layout.tsx` | Uses `getCachedUser()` and Supabase profile lookup. | Use app auth helper and Postgres profile repository. | Protected app shell route. | Open |
...
| `src/app/(recruiter)/admin/layout.tsx` | Uses `createClient()`, `getCachedUser()`, and `recruiter_profiles` to gate admin. | Use app auth helper and DB-backed roles. | Current admin behavior relies on hardcoded emails plus profile lookup. | Open |
| `src/app/(recruiter)/admin/feedback/page.tsx` | Uses `getCachedUser()`/`createClient()` for auth/profile timezone and repository factory for feedback admin rows. | Replace auth/profile timezone lookup with app session/profile helper after auth/profile migration. Feedback data can already select Postgres with `FEEDBACK_REPOSITORY_BACKEND=postgres`. | Admin-only data view; route-level authorization still depends on admin layout/RBAC migration. | In progress |
| `src/app/(recruiter)/recruiter/dev-eval/page.tsx` | Uses `getCachedUser()` for internal dev-eval route. | Use app auth helper or retire dev-eval route if not needed. | Dev-only surface may be removed rather than ported. | Open |
| `src/app/(recruiter)/recruiter/dev-eval/[id]/page.tsx` | Uses `getCachedUser()` plus repository factory for session reads. | Replace `getCachedUser()` with app auth helper or retire route. Session data can already select Postgres with `SESSION_REPOSITORY_BACKEND=postgres`. | Lower priority if dev-eval is removed. | In progress |
| `src/lib/auth/rbac.ts` | Imports Supabase `User`; hardcoded admin email allowlist. | Replace with app `CurrentUser` type and DB-backed roles (`admin`, `qa`, `recruiter`). | Temporary allowlist can bridge migration but should not be final. | Open |
...
| File | Supabase dependency | Replacement direction | Risk/notes | Status |
| --- | --- | --- | --- | --- |
| `src/app/api/invite/send/route.ts` | Calls `createClient().auth.getUser()` and records `missing_supabase_user`. | Replace with `requireCurrentUser()` and app-user ids. | Invite send is core recruiter flow. | Open |
| `src/app/api/invite/resend/route.ts` | Calls `createClient().auth.getUser()`. | Replace with app session auth. | Preserve resend authorization and logging. | Open |
| `src/app/api/questions/generate/route.ts` | Calls `createClient().auth.getUser()` before generating questions. | Replace with app session auth. | Core recruiter AI surface. | Open |
| `src/app/api/recruiter/invites/route.ts` | Calls `createClient().auth.getUser()`; invite repository now goes through `createInviteRepository()`. | Replace with app session auth. Postgres invite repository can be selected with `INVITE_REPOSITORY_BACKEND=postgres` once surrounding dependencies are ready. | Batch invite creation still depends on Supabase-backed auth, idempotency, and rate limit. | In progress |
| `src/app/api/recruiter/invites/[batch_id]/retry/route.ts` | Calls `createClient().auth.getUser()`; retry app default now goes through `createInviteRepository()`. | Replace with app session auth. Postgres invite repository can be selected with `INVITE_REPOSITORY_BACKEND=postgres` once surrounding dependencies are ready. | Must preserve batch ownership; idempotency still Supabase-backed. | In progress |
| `src/app/api/recruiter/ops/metrics/route.ts` | Calls `createClient().auth.getUser()` for GET/POST. | Replace with app session auth and admin/ops role checks. | Operational visibility should not be public. | Open |
...
| `src/lib/server/application/invites/resend-invite-email.ts` | Uses repository factory for marking invitation sent. | Complete for session repository selection. Continue auth/email branch reconciliation separately. | Resend path can select Supabase or Postgres by env. | Done |
...
| File | Supabase dependency | Replacement direction | Risk/notes | Status |
| --- | --- | --- | --- | --- |
| `src/app/login/page.tsx` | `createBrowserClient`; sign in/sign up. | App auth server action/API route. | Keep UI, replace plumbing. | Open |
| `src/components/auth/LogoutButton.tsx` | `createBrowserClient`; sign out. | App logout endpoint/action. | Simple but user-visible. | Open |
| `src/components/layout/RecruiterMobileDock.tsx` | `createBrowserClient`; sign out. | App logout endpoint/action and app user type. | Mobile route must match desktop. | Open |
| `src/components/auth/ProfileGuard.tsx` | Browser `getUser()` and DB query. | Server-backed profile state or app profile API. | Avoid browser DB access entirely. | Open |
...
| Backend selector env | `SESSION_REPOSITORY_BACKEND`, `INVITE_REPOSITORY_BACKEND`, `TEMPLATE_REPOSITORY_BACKEND`, `FEEDBACK_REPOSITORY_BACKEND`, `CANDIDATE_TOKEN_BACKEND`, `IDEMPOTENCY_BACKEND`, `RATE_LIMIT_BACKEND`, and `METRICS_BACKEND` now accept `postgres` in addition to migration fallback values. | Pin these to `postgres` in migrated environments and update production contract tests before final cutover. | Consider transitional Supabase compatibility only during migration. | In progress |
...
8. Replace browser Supabase client usage.
