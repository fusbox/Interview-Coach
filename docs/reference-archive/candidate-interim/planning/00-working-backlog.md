...
Date: 2026-05-08
Original status: Retired project artifact

## Purpose

This file was the mutable working document for candidate app execution from May 2026 through late June 2026 while the candidate app backlog was being assembled in the shared repo and mirrored manually into Azure Boards.

It is no longer the canonical item list. Current execution state should be read from:

- [HANDOFF](HANDOFF.md) for active execution context and next slices.
- [SPEC](SPEC.md) for product intent and candidate-facing behavior.
- [DATA_CONTRACT](DATA_CONTRACT.md) for durable system/data vocabulary.
- [Decision Records](08-decisions/README.md) for durable why-decisions.
- `docs/candidate-app/work-items.csv` for the latest Azure Boards export snapshot.

This retired backlog is preserved as a project artifact because it contains useful historical decomposition, execution rows, and local notes that may still explain why earlier dashboard, session, and quality work was shaped the way it was. Do not add new work items here. New Azure import files should continue using the gitignored `work-items-*.csv` convention described in [Azure DevOps Operating Model](07-ops/azure-devops-operating-model.md).
...
| SESS-S14 | 788 | Story | New | Fu Chen <fu@rangam.com> | Add candidate session transition loaders | `/practice` generation-to-session landing and session landing-to-active-question transitions show candidate-appropriate multistep loader states. The generation-to-session transition should adapt the multistep loader surface into a rotating coaching carousel that defines question categories and gives brief, memorable answer guidance while question/audio readiness completes |
...
| SESS-S19 | 803 | Story | Resolved | Fu Chen <fu@rangam.com> | Present question plan on candidate session landing | The candidate session landing screen explains the planned question mix for the target interview in candidate-facing language before the first question starts, without implying the generated session alone defines full interview preparedness |
| SESS-S20 | 804 | Story | Resolved | Fu Chen <fu@rangam.com> | Stop producing legacy readiness-level analysis metadata | New shared answer-analysis outputs no longer write legacy `meta.readinessLevel`; historical payloads remain tolerated for compatibility |
...
| DASH-S08 | 801 | Story | New | Fu Chen <fu@rangam.com> | Add optional target interview date affordance | Candidate setup and dashboard can progressively capture or edit an optional target interview date when interview-stage context is present, using the date only for pacing, reminders, and candidate-facing preparation context |
| DASH-S09 | 802 | Story | New | Fu Chen <fu@rangam.com> | Explore bite-sized practice paths | Dashboard should consider lower-friction practice paths, such as one-question targeted reps or skill-specific exercises, when those paths support the same preparedness objectives without forcing every next step through full session completion |
| DASH-S18 | 805 | Story | Resolved | Fu Chen <fu@rangam.com> | Preserve planned question category coverage before answers | Fresh active sessions keep generated/planned category coverage visible in the dashboard before any answer is scored; category states remain to-practice until practice evidence exists |
| DASH-S19 | 806 | Story | Resolved | Fu Chen <fu@rangam.com> | Drive Answer Skills ring from dimension-level preparedness | Dashboard Quick View Answer Skills child slices use dimension-level preparedness states from hidden score dimensions instead of inheriting parent lane state, with legacy/scoreless fallback preserved |
| DASH-S20 | 807 | Story | New | Fu Chen <fu@rangam.com> | Clarify Question Mix chart semantics | Dashboard Quick View Question Mix uses planned category distribution as the total pie shape, separates practiced and upcoming arcs, and applies preparedness-state color only where scored practice evidence exists |
| DASH-S21 | 808 | Story | New | Fu Chen <fu@rangam.com> | Seed dashboard practice-data scenarios for UI validation | Dev-only seed fixtures can create multiple fake candidates, active and paused sessions, partially answered plans, completed multi-round histories, and session-over-session evidence changes so dashboard UI can be validated without manually running full sessions |
| DASH-S22 | 809 | Story | Resolved | Fu Chen <fu@rangam.com> | Align Practice Next with planned coverage baseline | Dashboard Practice Next should compare selected target interview evidence against the persisted question plan baseline before recommending lower-scoring matrix improvement cells, so follow-up practice stays aligned with the same stage/count plan used to generate questions |
| DASH-S23 | 810 | Story | Resolved | Fu Chen <fu@rangam.com> | Move Quick View interactions onto pie charts | Answer Skills and Question Mix pie slices inherit the prior lane/category hover and open-detail behavior, and the duplicate lower lane/category cards are removed from Quick View |
| DASH-S24 | 811 | Story | Active | Fu Chen <fu@rangam.com> | Add Quick View guidance-card interaction parity | Quick View should make the overall read the default guidance state, reveal segment-specific guidance on hover/focus or first mobile tap, revert to overall read on mobile tapaway, open drilldown modals directly from pie segments, support outer skill-dimension segment guidance, and place the guidance copy in a full-width lower guidance card that works when charts are side-by-side or stacked |
...
> Historical dashboard direction: items 186-200 record the pre-rebuild
> score-driven prototype arc. They are not governing V2 work even where their
> original status remains `Active`. ADR-0010 and the Evidence-First Dashboard
> Information Architecture retire numeric score rollups, opaque overall reads,
> and legacy snapshot fallbacks. Current execution is sequenced in `HANDOFF.md`.
...
| 198 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Render dashboard preparedness matrix | Dashboard now renders the Preparedness Map as an answer-performance lane by question-category matrix, carries lane-specific category score states in the read model, and reuses existing Q/A drilldowns for row, column, and cell evidence views |
| 199 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Flip dashboard matrix axes and reduce cell noise | Preparedness Map matrix now renders question categories as rows and Substance, Structure, and Delivery as fixed columns, removes lane state labels from the headers, and removes practiced/upcoming text from individual cells so the grid is more compact on small screens |
| 200 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Stage instant-read dashboard surface | SPEC and handoff now allow a front-facing, non-score preparedness snapshot before the evidence-backed matrix; the current prototype removes category text labels from the constellation front while preserving category markers as quieter evidence-distribution cues |
| 201 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Align candidate shell dimensions | Candidate desktop sidebar now uses the recruiter shell width and candidate app-page content uses the recruiter-style responsive padding rhythm while preserving the simpler candidate navigation model |
| 202 | New | SESS-S14 | 788 | Fu Chen <fu@rangam.com> | Refine transition loaders into coaching carousel | Existing transition-loader story now targets a generation-to-session carousel that cycles through question-category definitions and plain-spoken coaching tips while generation/audio readiness completes |
| 203 | New | SESS-S19 | 800 | Fu Chen <fu@rangam.com> | Stage candidate landing question plan | Candidate session landing should eventually present the planned question mix in candidate-facing language before Q1 begins |
| 204 | New | DASH-S08 | 801 | Fu Chen <fu@rangam.com> | Stage optional target date | Candidate setup/dashboard should eventually capture an optional target interview date for pacing and preparation context when stage context makes it useful |
| 205 | New | DASH-S09 | 802 | Fu Chen <fu@rangam.com> | Stage bite-sized practice paths | Dashboard should explore smaller targeted practice paths that can support the same skill-building objectives without over-indexing on full-session completion |
| 206 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Tighten matrix axis affordances | Preparedness Map matrix now keeps the target-interview title full width, delays the sidebar stack breakpoint until the matrix has room, keeps column headers one-line, fixes matrix row height so row-label wrapping does not resize rows, forces transparent outer cell borders with quiet gray not-practiced cells, and uses a violet continuous row/column underlay only when hovering/focusing row or column headers |
| 207 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Define question category contract | Added the Question Category Contract to define Screening, Behavioral, Culture/Fit, Scenario, and Technical/Role-Specific as interview-demand indicators, including candidate-facing definitions, interviewer objectives, resume/JD usage, coach evidence, weak patterns, feedforward/feedback indicators, dashboard usage, and visualization implications |
| 208 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Land first instant-read dashboard snapshot | Dashboard now derives and renders a qualitative instant-read preparedness snapshot above the evidence matrix, using the same selected-target evidence scope for an overall coach read, three lane nodes, and category coverage marks. Snapshot lane/category tap targets reuse existing drilldowns, while the front/back or toggle interaction remains the next product decision. |
| 209 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Land instant-read view boundary | Dashboard Preparedness Map now renders through one `PreparednessMapExperience` boundary with Quick View as the default view and Details as an explicit toggle, preserving existing lane/category/cell drilldowns while preventing the snapshot and matrix from reading as two unrelated dashboard sections |
| 210 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Mature instant-read candidate-facing copy | Dashboard Quick View now avoids candidate-visible implementation language such as evidence map and signal, uses short toggle labels to prevent overflow, softens the decorative snapshot lines, and reframes helper copy around practice, question types, and next focus |
| 211 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Shift instant-read surface toward SVG coach compass | Quick View now follows the coach-read compass direction: the SVG layer draws the visible lane-node halos, state marks, and connector geometry for Substance, Structure, and Delivery, while HTML controls remain as accessible tap targets and labels. Mobile keeps a stacked control layout for readability; desktop no longer relies on large card blocks as the primary visual language. |
| 212 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Add instant-read selected coach read | Quick View now updates the central coach read when a candidate hovers or keyboard-focuses a lane or question type. The selected read gives a short candidate-facing interpretation while clicking/tapping still opens the existing drilldown modal, preserving one evidence system and avoiding new scoring or persistence. |
| 213 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Polish instant-read selected-read loop | Quick View now makes the selected-read loop explicit with visible usage copy, pointer/focus-triggered SVG emphasis, aria-live read updates, and a Return to overall read control. This keeps the compass interaction accessible while preserving existing lane/category drilldown modals as the only evidence-detail path. |
| 214 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Add selected-read details action | Quick View selected reads now include an Open details action for the selected lane or question type. The action uses the same existing lane/category drilldown handlers as the visual node controls so the central coach read becomes a useful interpretation surface without creating parallel evidence UI. |
| 215 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Start instant-read constellation walkthrough | Quick View now starts as a closed coach-read orientation object, opens into the preparedness map, and uses tap/click selection as the primary path before evidence drilldown. Selecting a lane or question type updates the center read; the center Open details action opens the existing lane/category evidence modal. |
| 216 | Resolved | SESS-S19 | 803 | Fu Chen <fu@rangam.com> | Show question plan snapshot on candidate session landing | Candidate session landing now reads persisted `questionPlanSnapshot` and shows the resolved stage/count and nonzero category counts before Q1 starts |
| 217 | Resolved | SESS-S20 | 804 | Fu Chen <fu@rangam.com> | Stop producing legacy readiness-level analysis metadata | New shared answer-analysis outputs no longer write legacy `meta.readinessLevel`; historical rows remain schema-tolerated only |
| 218 | Resolved | DASH-S18 | 805 | Fu Chen <fu@rangam.com> | Preserve planned question category coverage before answers | Dashboard Quick View and Details can render planned/generated category coverage for active sessions before answer scoring, without treating upcoming questions as zero-score evidence |
| 219 | Resolved | DASH-S19 | 806 | Fu Chen <fu@rangam.com> | Drive instant-read child skills from dimension state | Score-driven prepProfile read model now exposes child dimension state for each release lane, and Dashboard Quick View Answer Skills ring uses those child states while preserving parent-lane fallback for legacy/scoreless rows |
| 220 | Active | DASH-S20 | 807 | Fu Chen <fu@rangam.com> | Clarify Question Mix chart semantics | Dashboard Quick View Question Mix now treats planned category count as the total segment distribution, splits each category into practiced and upcoming arcs, colors practiced arcs by preparedness state, and keeps upcoming arcs muted so unanswered planned questions do not read as weak performance |
| 221 | Resolved | SESS-S19 | 803 | Fu Chen <fu@rangam.com> | Fix landing practice-plan persistence boundary | `PostgresSessionRepository.update()` now preserves arbitrary `session.intakeData` while overlaying repository-managed fields, so candidate-created sessions keep `questionPlanSnapshot` through the database round trip and the landing page can render the practice plan |
| 222 | Active | DASH-F03 | 774 | Fu Chen <fu@rangam.com> | Expose full Practice Next item list | Dashboard Practice Next now lists active-session upcoming planned questions first and falls back to all non-strong Preparedness Map cells when there is no active session, so the recommendation surface can be validated without hiding lower-priority items |
| 223 | Resolved | DASH-S22 | 809 | Fu Chen <fu@rangam.com> | Add practice coverage baseline to Practice Next | Added `PracticeCoverageBaseline` derived from the shared `QuestionPlan`, parsed persisted `questionPlanSnapshot` in the dashboard loader, and now show completed-session coverage gaps before lower-scoring matrix improvement cells |
| 224 | Resolved | DASH-S23 | 810 | Fu Chen <fu@rangam.com> | Move Quick View interactions onto pie charts | Quick View now removes the duplicate lower skill/category cards. Answer Skills and Question Mix pie slices update the central coach read on hover/focus and open the existing lane/category drilldowns on click/tap, while the empty preview keeps the same chart grammar without opening details. |
| 225 | Active | DASH-S24 | 811 | Fu Chen <fu@rangam.com> | Add Quick View guidance-card interaction parity | Next Quick View pass should make overall read the default state, move hover/tap guidance into a full-width lower guidance card, support outer skill-dimension segment guidance with hardcoded user-facing definitions, use primary lifted/glowing focus states plus secondary parent/sibling focus states, and use mobile first-tap reveal, tapaway reset, plus second-tap modal open where direct hover parity is unavailable. |
| 226 | Active | DASH-S24 | 811 | Fu Chen <fu@rangam.com> | Fix Quick View persistent default and state-colored focus | Parent lane slices now carry their preparedness state into focus styling so green segments stay green on hover/focus. Quick View clears selected guidance back to the overall read on segment mouseout, surface blur, Escape, and mobile tapaway while preserving keyboard focus behavior. |
| 227 | Active | DASH-S24 | 811 | Fu Chen <fu@rangam.com> | Move Quick View modal open onto chart segments | Quick View chart segments now own the modal-opening behavior formerly handled by the selected-read action button. Parent lane slices, outer skill-dimension slices, and question mix slices open the existing lane/category drilldown modals directly, with full-page tests covering each path. |
| 228 | Resolved | DASH-S25 | TBD | Fu Chen <fu@rangam.com> | Show baseline coverage in dashboard visuals | Question Mix and Details matrix now merge `PracticeCoverageBaseline` category minimums into their category model, so planned-but-unpracticed categories appear as to-practice coverage instead of only surfacing in Practice Next. Active Practice Next still lists only actual unanswered current-round questions. |
| 229 | Resolved | QSO-S21 | 812 | Fu Chen <fu@rangam.com> | Persist defined rigor baseline separately from round plan | Candidate session creation now persists `rigorBaselineSnapshot` beside `questionPlanSnapshot`. The selected round stays sized to the candidate-chosen count, while dashboard coverage prefers the stage-defined baseline snapshot and falls back to the round plan for older rows. |
| 230 | Resolved | DASH-S26 | 813 | Fu Chen <fu@rangam.com> | Tighten mobile dashboard header and prep context switcher | Dashboard now uses a static `Practice Coach` page title, removes active/completed count text from prep-context pills, and constrains long target-role labels inside a horizontal switcher so the mobile viewport does not widen. |
...
| 796 | Active | SESS-F05 | 777 | Fu Chen <fu@rangam.com> | Add recruiter interview-detail planning controls | Recruiter `/recruiter/create` now exposes interview stage and question count before question entry, replaces the extra Add Questions step with the AI/manual button group, uses recruiter-specific stage labels, passes stage/count to the shared question-generation boundary, shows a category distribution confirmation, enforces the confirmed question plan for both AI and manual entry, repairs schema-valid provider output that under-fills planned category slots, preserves extra generated keyed questions through recruiter mapping, restores the accepted setup banner/question editor when returning from later steps, updates confirm-preview configuration to the five plain-language categories, aligns question-confirmation/post-send modal styling with the multistep loader surface, and updates shared question generation to a QuestionPlan-first prompt/schema contract while preserving recruiter-invited feedback behavior |
| 797 | Active | DRFT-F04 | 772 | Fu Chen <fu@rangam.com> | Promote candidate interview details setup controls | `/practice` now promotes interview stage and question count out of Advanced setup into a first-class Interview Details section, aligns the controls with recruiter planning styling, merges not-sure/no-interview-scheduled into one balanced-practice card stored as `practice_only`, and hides Advanced setup until future intake/customization controls land. Browser validation is still needed before resolving. |
| 798 | Active | QSO-F04 | 775 | Fu Chen <fu@rangam.com> | Contain recruiter template drift after planned question setup | Recruiter create now moves template use next to AI/manual question entry as a Use a Template button, disables it when no templates exist, opens a clickaway/tapaway modal listing template name, target role, category counts, and created date, and applies templates with a Template questions loaded banner. Template use from `/recruiter/templates` now enters the same banner/editor state, and template-loaded editors show only categories that contain template questions. AI-generated and template-loaded question fields are read-only; manual-entry fields remain editable and the manual banner prompts recruiters to enter questions from the approved question plan. Clear buttons were removed from the editor. Template cards now show the five plain-language category chips. Longer-term v2 template shape remains open. |
| 799 | New | QSO-F04 | 775 | Fu Chen <fu@rangam.com> | Decide recruiter resume-aware question generation sequence | Resume-aware question generation for recruiter invites is parked for later because batch invites can involve multiple candidates and resume context is not obviously a single upstream input for the create flow. Current resume context still supports invited-session hints, strong responses, feedback, and summaries downstream. |
