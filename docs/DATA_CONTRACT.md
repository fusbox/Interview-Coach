...
Candidate active-use reporting is a separate append-only `candidate_engagement_slices` boundary. Each row is owned by the composite candidate practice-session/profile identity and carries a client-generated slice id, tracker-instance id, local sequence, bounded active milliseconds, client interval bounds, allowlisted window/activity/flush reason codes, and server receipt time. Exact replay is idempotent. The ledger must not store free-form interaction detail, question or answer content, transcript, resume/JD content, coaching, assistance, evaluator or provider data, IP address, or user agent. Engagement totals are derived from accepted slices; neither `candidate_practice_sessions` nor legacy `sessions.intake_json.engaged_time_seconds` is the V2 authority. Candidate/session deletion cascades through slices. Invited-practice engagement remains a separate future owner-scoped contract.
...
- Initial setup resolves ownership and prep context, creates one deterministic canonical plan, and invokes the selected question-wording runtime exactly once before session insertion. The effective canonical count is `max(stageRecommendedQuestionCount, selectedQuestionCount)`; the selected count is separately persisted as `paceSize`. The baseline, initial-session plan, and wording snapshots contain the same complete ordered question set, and every slot is canonical baseline coverage. Only an accepted exact slot/order/category mapping may be stored as `questionWordingSnapshot`; its immutable generation identity is part of that same session JSONB snapshot. Provider failure creates no session and consumes no trusted host setup staging. A prep context with no session may be reused by an explicit retry as partial-write repair. Follow-up practice after initial-plan completion snapshots exact selected source questions and must not invoke the wording provider.
...
- `/candidate/session/[sessionId]/progress` may save the active session view state to `candidate_practice_sessions.progress_state_json` for candidate-owned durable sessions. Current progress states are `planned`, `question_preview`, `live_question`, and `completed`; question-surface and completed states must carry the current question index. The first-pass current index is normalized through the common canonical next-unanswered resolver; it skips durably answered slots and wraps once from an explicit focus. `pace=one` and the persisted setup `paceSize` bound newly settled questions for the current visit, then return to the dashboard without completing the session. This supports pause/resume, refresh, cross-tab recovery, paced exits, and final canonical-session completion without creating continuation sessions. The completion command independently rejects an unfinished canonical set with `CANONICAL_PLAN_INCOMPLETE`; client routing is not the completion authority.
...
- `/candidate/session/[sessionId]/coach-update/repair` is the ownership-scoped repair boundary. With a server-rendered question checkpoint, it may retry synthesis for that exact active-session question without accepting answer, evaluator, provider, or prep-context identity from the browser. The legacy completed-session path may still run bounded answer-analysis repair: it loads the candidate-owned completed session, immutable latest answer attempts, and evaluator-run history; rejects cross-owner evidence; and invokes the existing answer-analysis boundary only for answered occurrences whose exact latest immutable attempt is recoverable or retryable under the current serving configuration. A completed-session request attempts no more than two eligible occurrences. Fresh pending claims, accepted results, nonretryable terminal results, malformed completed results, and generation-capped attempts do not create a provider call. Concurrent or repeated requests converge through the ordinary per-answer evaluator claim and deterministic analysis idempotency record. The response and diagnostics expose only bounded status and counts. It never rewrites answers, completion snapshots, prior evaluator generations, or creates partial Coach Update content. Session completion may invoke analysis repair but does not create a second Coach Update; dashboard `GET` remains side-effect free.
...
- `candidate_coach_update_artifact` is the versioned synthesis contract for one successfully settled candidate answer. The active writer is keyed by candidate and opaque prep-context ownership, source session and local question key, immutable answer-attempt id, accepted evaluator-run id, question-checkpoint fingerprint, synthesis input fingerprint, provider/model/prompt/evaluator versions, generation attempt, lifecycle, validation metadata, and candidate-safe rendered content. A question is eligible only after the latest answer attempt and its accepted `candidate_coaching` run are durable and a terminal feedback action advances, finishes, retries, pauses, or deliberately skips onward. Requested, failed, rejected, stale, cross-candidate, cross-context, superseded, skipped, or unanswered facts are excluded. Generation failure never rolls back the answer, feedback action, or cursor. The service claims generation under a question-checkpoint lock, replays an existing requested/completed same-input claim, expires an abandoned requested claim after its bounded lease, permits a new generation attempt after terminal failure, re-reads source facts before completion, and rejects changed fingerprints. A completed read requires an accepted validation disposition and exact versioned candidate-safe content with no undeclared or score-like fields. Historical session-level artifacts remain readable but the active runtime no longer waits for session completion or rewrites them. The artifact preserves stable replay and QA lineage but is not durable preparedness truth.
...
- `coachGuidedFocus`: feedback-based practice for the selected target interview context, sourced from candidate-safe coaching on a submitted answer.
  ...
  - When the latest completed round contains several coached questions, the read ranks their canonical questions by latest-attempt posture: incomplete, Emerging, Clear, Strong, then evaluation unavailable. Persisted round order is the deterministic tie-break, not the recommendation rule. If preparedness history is unavailable, the read preserves the round's accepted coaching order rather than inventing a band.
