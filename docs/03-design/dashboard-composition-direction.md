...
- candidate initials as the identity anchor;
...
- The dashboard canvas remains a review decision rather than a ratified production surface. The Phase 1 lab provides three switchable studies beneath the same content: `Diagonal confluence`, `Cool horizon`, and `Peripheral mesh`. `Diagonal confluence` is the current leading hypothesis and uses a distinctly washed, high-lightness powder-blue and cool-green field rather than a saturated backdrop or transparent color over a neutral base. Every light-theme stop—including the stronger corner fields, center bridge, and base layer—is lifted and reduced in chroma together so cards remain the primary color-bearing surfaces. The green remains recognizable as cool mint or softened viridian while avoiding saturation and yellow-green warmth. Its dark translation remains the existing lifted, low-intensity blue-green dusk; this light-theme calibration does not change dark mode. The canvas continues through the transparent dashboard header; only the role switcher and practice-queue control use restrained glass material so their labels and actions remain legible against the wash. The comparison studies retain their existing canvas formulas while sharing this header treatment. These are background-layer hypotheses only; they do not assign semantic meaning to the green or change any card, stage, progress, or interaction state.
