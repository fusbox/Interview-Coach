...
3. Check whether recruiter or QA routes are also affected, especially `/recruiter`, `/recruiter/create`, `/recruiter/settings`, and `/qa/ai-eval`.
...
- candidate quick-link reaches `/candidate/launch` but does not establish a clean candidate route
- launch returns a generic 4xx/5xx or repeatedly returns to the host
- `/candidate/setup` or `/candidate/dashboard` cannot resolve the app-owned candidate session
- candidate route accepts the wrong actor session

Checks:

- Confirm `NEXT_PUBLIC_APP_URL=https://interviewcoach.talentarbor.com`.
- Confirm production host-launch secret, expected issuer/workspace, app-session TTL, `DATABASE_URL`, and bounded TA MSSQL configuration are present in the secret/configuration store.
- Correlate the random launch request id with the allowlisted `assembly`, `verification`, or `exchange` outcome. Never capture the token-bearing URL.
- Confirm token lifetime, issuer/product/source claims, one-time exchange, host candidate lookup, Postgres session creation, cookie attributes, and canonical redirect in that order.
- Verify `/candidate/dev/launch` is unavailable in production.

Containment:

- Ask the TalentArbor owner to disable or hide the affected quick-link if launch is broadly broken.
- Do not weaken signature, claim, replay, ownership, or expiry validation during an incident.
