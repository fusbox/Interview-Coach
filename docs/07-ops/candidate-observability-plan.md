...
| Candidate-led active engagement | append-only active-time slices and an administrator-only aggregate report | Validate active practice use and support future BI without inferring from wall-clock session duration | Store bounded duration and allowlisted reason codes only; no free-form event details, question/answer/transcript, coaching, resume/JD, prompt/provider, IP, or user-agent data. Production collection remains explicitly gated pending privacy approval. |
