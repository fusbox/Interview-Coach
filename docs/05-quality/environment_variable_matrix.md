...
| `METRICS_BACKEND` | Optional locally, recommended when enabling durable metrics in staging/production | Server only | Metrics sink selection | Supported values: `memory`, `supabase`. Current implementation defaults to `memory` unless explicitly set to `supabase`. Use `supabase` only after the metrics rollup migration is applied. |
...
- `METRICS_BACKEND` should remain `memory` until the durable metrics migration is applied in the target environment and the rollout is intentionally enabled.
