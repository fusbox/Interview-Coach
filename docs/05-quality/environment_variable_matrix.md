...
| `NEXT_PUBLIC_BASE_URL` | Optional | Server-only use in current code path, but public-prefixed | Email logo/debrief link generation | May still be used as a secondary configured origin source in some paths, but it should not be the sole production trust contract for candidate-facing invite origin generation. |
| `NEXT_PUBLIC_APP_URL` | Required for production release | Client + server | Canonical app origin for invite links and trusted public URL generation | This is the intended trusted production origin. Production contract should fail fast when it is absent, and production URL generation should not rely on request-derived host fallback. |
