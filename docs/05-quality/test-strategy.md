...
The candidate session component tests cover the recruiter-style session workspace adapted for authenticated candidates: progress header, prompt shell, typed answer submission, saved-answer coaching, retry/continue controls, and pause/resume/completed states. Candidate-led engagement coverage separately proves the generous legacy Tier 2/Tier 3 window behavior, visibility and single-tab gates, monotonic elapsed-time accounting, idempotent persistence, ownership denial, and the development-only hidden inspector. The inspector exposes engagement state and a local event/window log only; the stale AI-prompt-context affordance is retired. The negative-path cadence includes finish failure followed by answer retry: the failed finish remains retryable, but its transient message is cleared before the linked retry is submitted and must not appear in that new attempt's feedback.
