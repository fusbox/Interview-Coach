...
- exact `google_genai` / `google_gemini_2_5_flash_coach_update_v4` selection;
- one fixed synthetic case containing first-practice and repeat-practice coaching facts;
- one provider transport attempt for an accepted first draft, or one bounded second attempt only when candidate-language validation requests a rewrite, with exact model, schema, timeout, and generation settings;
...
5. `CANDIDATE_COACH_UPDATE_PROFILE=google_gemini_2_5_flash_coach_update_v4`;
...
$env:CANDIDATE_COACH_UPDATE_PROFILE="google_gemini_2_5_flash_coach_update_v4"
...
- bounded call count, aggregate latency, and aggregate token counts;
