...
The code-owned profile is `google_gemini_2_5_flash_v1`. Slice 119 first landed adapter version `google_genai_evidence_first_adapter_v1`; credentialed and manual-browser findings advanced the accepted V9 calibration to `google_genai_evidence_first_adapter_v10`. Slice 189 staged prompt bundle V10 and adapter V11. Slice 190 advanced through V11/adapter V12 to V12/adapter V13, adding bounded generated-language repair and completing the focused V6 run without an execution failure. V13/adapter V14 first made intervention posture code-owned. Slice 192 stages `candidate_evidence_first_prompts_v14` with adapter `google_genai_evidence_first_adapter_v15`. V14 preserves the generated-language repair, derives `move_on` / `polish` / `remediate` from the deterministic question-preparedness result, and permits whole-answer category-signal evidence when no reliable exact span exists. Application code still reattaches the exact intervention, signal, anchor, and allowed content before provider-neutral validation. No credentialed provider promotion is claimed before the production UI and live candidate runthroughs are ready.
