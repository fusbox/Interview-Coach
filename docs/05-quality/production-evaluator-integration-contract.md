...
The code-owned profile is `google_gemini_2_5_flash_v1`. Slice 119 first landed adapter version `google_genai_evidence_first_adapter_v1`; credentialed and manual-browser findings advanced the accepted V9 calibration to `google_genai_evidence_first_adapter_v10`. Slice 189 staged prompt bundle V10 and adapter V11. Slice 190 advanced through V11/adapter V12 to V12/adapter V13, adding bounded generated-language repair and completing the focused V6 run without an execution failure. V13/adapter V14 first made intervention posture code-owned. Slice 192 staged `candidate_evidence_first_prompts_v14` with adapter `google_genai_evidence_first_adapter_v15`. Slice 206 stages `candidate_evidence_first_prompts_v15` with adapter `google_genai_evidence_first_adapter_v16`; it preserves V14 evaluation and intervention meaning while making the acknowledgement conversational, evidence-grounded, and posture-aware. Application code still reattaches the exact intervention, signal, anchor, and allowed content before provider-neutral validation. No credentialed provider promotion is claimed before representative live candidate runthroughs.
...
Failed live and browser attempts were treated as interface evidence. Gemini rejected the original over-complex serving schema, and accepted generations showed that model-authored offsets, derived markers, missing-evidence codes, immutable envelope identity, intervention discriminants, observed-signal citation cleanup, ambiguous verifier field names, unsupported technical verdicts without references, prose length compliance, and delivery-guidance placement were brittle or redundant. The production boundary asks the model for exact quotes, bounded semantic classifications, and candidate-facing language. Application code reattaches immutable identity, computes exact offsets, clears citations from unobserved signals, derives grounded markers and missing-evidence codes, owns sensitive-intervention anchors, forces technical accuracy to `not_assessed` when no trusted reference was supplied, normalizes usable affirm-plus-upgrade feedback to a polish intervention, sentence/word-clips generated prose to contract bounds, distinguishes grounded candidate outcomes from coach-owned scoring, rejects delivery-mechanics guidance outside a supported voice-only note, records content-free schema issue paths, and validates the strict internal contract. Verifier tasks distinguish review triggers from unsupported-conclusion reasons and explicitly treat a correctly extracted contradiction as support for the extractor conclusion rather than the candidate claim. Request-affecting changes advanced the evaluator contract to `candidate_evidence_first_v2`, prompt bundle to `candidate_evidence_first_prompts_v8`, and adapter to `google_genai_evidence_first_adapter_v10`.
...
## Slice 162 Marker-To-Criterion Calibration

The initial `candidate_evaluator_golden_v1` credentialed artifacts remain the historical evidence that supported first-profile ratification. The active source gate is now `candidate_evaluator_golden_v2`: twelve fixed cases span all five question categories and add concise-sufficient screening, behavioral team-result-without-personal-action, scenario-without-problem-framing, generic culture/fit, and strong-content/non-native-grammar behavior.

At the Slice 162 milestone, every case asserted category signals plus the applicability and allowed band of all five universal criteria. The deterministic mapper also prevented `answerUsability: thin` from acquiring `clear` or `strong` criteria from isolated markers; every assessable criterion remained `observed/emerging`, while technical role skill was `unscoreable` without a trusted reference. Slice 189 supersedes only that no-reference role-skill posture: answer-demonstrated role capability is now independently assessable, while technical correctness remains `not_assessed` without a trusted reference. A concise usable logistics answer remains independently protected so brevity itself is not penalized.

The expanded network-free gate is implemented. Existing v1 artifacts must not be relabeled as v2 evidence; Slice 163 below records the separate credentialed v2 evidence and every request-affecting correction advanced immutable configuration identity before rerun.

## Slice 163 Expanded Credentialed Calibration

The twelve-case gate exposed and corrected category-aware applicability, scenario role-skill over-credit, overly rigid golden facts, and unsupported typed-answer delivery coaching. The final serving request is evaluator `candidate_evidence_first_v2`, prompt bundle `candidate_evidence_first_prompts_v8`, Google adapter `google_genai_evidence_first_adapter_v10`, and fingerprint `69a65121b67ec1b9745d50c6d952409ea4291e4a7d70b6ca423966eb667c980f`.

Artifacts `live_eval_c1da8f7f7104975c` and `live_eval_16784048b2e5d6ca` each accepted and passed all 12 cases. Offline comparison `live_compare_29f03288ef5a9e51` found all 12 inputs comparable. Human review accepted the final marker/signal/applicability/band/intervention meaning and both candidate-safe projections for every case; generated preference remains `not_reviewed` because artifact generation does not confer promotion authority. No second provider profile, technical-reference retrieval, reviewer UI, or candidate-facing criterion/band presentation was added.
