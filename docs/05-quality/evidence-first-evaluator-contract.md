...
This contract turns one immutable submitted answer attempt into candidate-safe coaching without asking a model to make an opaque overall judgment. It is the V2 replacement for V1's score-centered analysis truth. Application code may additionally derive the transparent qualitative question result defined by the [Question Preparedness Progress Contract](../04-architecture/question-preparedness-progress-contract.md); that result is not model-authored and is not an interview-readiness score. The executable schemas and validators live in:

- `src/features/evaluation-v2/evidence-first-evaluator-contract.ts`;
- `src/features/evaluation-v2/evidence-first-evaluator.ts`.

This contract was ratified on 2026-07-14. The provider-neutral runtime and local fixture exercise the same typed evaluation case, extractor task, deterministic validation/appraisal, conditional verifier task, feedback-composer task, retry budget, and candidate-safe projection boundary. The fixture supplies deterministic stage adapters; it is not a production model. The code-owned Google Gen AI adapter and pinned Gemini profile have now passed mocked conformance, credentialed synthetic gates, same-profile repeatability comparison, and disposable-DB candidate-route reconciliation. The candidate answer-analysis route selects that runtime only from the exact server provider/profile/key contract and only after ownership and durable evaluator-run claim authorization. Production reference authoring and retrieval are deferred for this release; the optional supplied-reference path remains strict. Production observability, organizational deployment readiness, and serving-profile promotion remain separate fail-closed work.
...
V15 is the only active prompt bundle for new evaluator work. Persisted V14 accepted runs remain readable through a separate historical compatibility boundary because V15 preserved their evaluation and intervention semantics; the stored V14 version remains intact as provenance and no V14 prompt is loaded at runtime. Earlier prompt bundles are not admitted by that boundary without an explicit schema-and-semantics review.
...
An observed category signal may use exact `span` evidence or a `whole_answer` evidence basis when the response meaning supports the signal but no unique exact highlight is reliable. A not-observed signal uses an `absence` basis. Whole-answer and absence signals may inform deterministic appraisal and adjacent indicators, but they never authorize an invented transcript range. Exact-span feedback claims and inline transcript annotations retain their stricter provenance rules.
...
Missing, unelicited, private, non-answer, or unclear-transcript evidence must not be converted to `emerging`. `Emerging` is an observed qualitative band, not a synonym for missing evidence. A `thin` answer is the bounded exception because it contains an answer that can be coached but not enough developed evidence to promote: every assessable criterion remains `observed/emerging`, regardless of an isolated direct-answer marker. Technical/role-specific `role_skill_signal` is appraised from exact answer evidence about relevant knowledge, application, reasoning, and verification awareness; it is not qualification inference and does not require a technical reference. When trusted reference evidence contradicts the answer, organization may remain independently appraisable but is capped at `clear`; a cluster of false technical assertions cannot become `strong` organization merely because extraction marks several technical signals. The bands are internal evaluator facts and contain no numeric score.
...
Application code derives one question-preparedness result after appraisal and uses that result to select the coaching posture. The internal criterion transformation is deterministic, non-persisted, and never exposed as a number:

| Derived attempt result | Posture | Intervention contract |
|---|---|---|
| `emerging` | `remediate` | Name the accepted gap and offer a retry path. |
| `clear` | `polish` | Offer at most one optional refinement, then continue. |
| `strong` | `move_on` | Affirm the grounded response pattern and continue without an upgrade. |
| `incomplete` | `remediate` | Build or recover the missing answer signal without inventing a strength. |
| Sensitive disclosure | `remediate` | Use the dedicated professional privacy reframe. |

`Strong` means the answer is good enough to stop improving in this interaction. `Clear` means a refinement can be useful but must not be framed as necessary remediation. `Emerging` means the accepted evidence supports a concrete remediation opportunity. `Incomplete` means the evidence cannot support a defensible question band. One unavailable criterion caps the result at Clear; two or more make it incomplete. A trusted technical contradiction caps the result at Emerging, while `technicalAccuracy: not_assessed` is neutral. A model cannot change this posture, stack several otherwise acceptable observations into a weaker valence, or add a required improvement to `move_on`.

Code may conservatively normalize a provider-authored `usable` answer to `thin` when a non-screening answer is very short and contains no grounded development evidence appropriate to its category. A generic preference or direct answer alone is not enough to avoid this normalization. Concise screening and logistics answers retain their category-specific brevity contract.

## Technical Correctness

The evaluator may emit `supported` or `contradicted` technical accuracy only when the case supplies a versioned technical reference with known concept ids. Otherwise technical accuracy is `not_assessed`. The provider adapter enforces this boundary after generation by replacing any provider-authored technical verdict and references with `not_assessed` and empty reference/evidence ids when the request carried no trusted technical reference.

Technical correctness and role-skill demonstration are separate facts. Without a reference, the extractor may observe that the answer used role-relevant knowledge, described practical application, explained reasoning, or showed verification awareness. Code may appraise `role_skill_signal` from those accepted answer spans, but neither the model nor application may call the knowledge correct, infer qualification from the JD/resume, or issue an authoritative factual correction. Model knowledge may support relevance and plausibility for calibrated coaching; it is not persisted or presented as verified truth.

Contradicted technical claims require verification before feedback composition. Partial concept coverage also triggers verification. A contradicted reference-grounded claim caps `role_skill_signal` at `emerging` when the contradicted evidence is the only support. The optional future technical-reference adapter must define source ownership, versioning, role-family coverage, expiry, and what happens when references disagree. Production reference authoring and retrieval are deferred for this release.

Technical category signals and technical quality are separate facts. The extractor may correctly observe that a candidate addressed a tradeoff while the trusted-reference comparison finds the stated tradeoff false. The appraisal retains that evidence lineage, but a signal supported only by contradicted spans cannot promote `impact_judgment_takeaway` above `emerging`. Correct technical impact evidence in other, non-contradicted spans may still contribute normally.
...
The hidden feedback plan carries the central read, signal posture, primary anchor, and intervention. Code derives an exact completion directive from the accepted appraisal before composition. The model receives that directive and writes within it; hydration reattaches the code-owned intervention, valence, detectability, and anchor before validation. `move_on` forbids an upgrade, redo prompt, or answer-pattern prescription. `polish` permits one biggest upgrade but no retry demand. `remediate` requires the applicable upgrade and redo content. The prompt keeps candidate-facing prose concise, but hydration and the candidate-safe projection preserve the provider's complete accepted text rather than destructively clipping it to a character count. Hidden plan prose may remain bounded because it never crosses the candidate UI boundary. Missing or incompatible intervention content remains fail-closed. Candidate feedback may carry:
...
An accepted evaluator-run result is the internal record. It contains accepted extraction, deterministic criteria and pattern gap, conditional verifier result, accepted feedback composition, candidate-safe feedback, stage-attempt metadata, aggregate latency/token totals, and explicit no-prompt/no-raw-output retention markers. The session snapshot is a separate bounded V2 projection containing answer identity, candidate-facing coaching, the minimal interaction directive needed to render retry/continue behavior, and the qualitative appraisal facts required by downstream candidate and Coach Update reads: answer-usability status, technical-accuracy status, criterion applicability/bands, the code-owned question-preparedness result, and the selected pattern gap. It excludes raw spans, verifier output, hidden feedback-plan prose, prompts, raw model output, and all numeric score compatibility fields. Downstream code consumes these accepted qualitative facts directly. It must not reconstruct an opaque model score, but it may derive the versioned question/prep-context progress projection from immutable accepted facts.
...
- Retire: a model directly judging the whole answer, numeric score averages as candidate truth, weakest-criterion-wins remediation, missing evidence mapped to low performance, app-source distinctions for the shared evaluator job, and legacy feedback prose as durable evidence.
...
2. Superseded in Slice 192: keep all five universal criterion appraisals, then derive one transparent question-preparedness result from the rated criteria. Immediate coaching uses that synthesized result instead of allowing one weaker criterion to force remediation.
