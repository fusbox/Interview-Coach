...
- `candidate_id`: `353373`
- `email`: `amitkumar+25june26@rangam.com`
- `product`: `interview-coach`
- `iss`: configured expected issuer
- numeric `iat` / `exp` / unique `jti`

The minted email must match `CandidateMaster.Email` for that candidate id (case-insensitive) or `/candidate/launch` fails closed. If the local TA row for `353373` uses a different email, override with `HOST_LAUNCH_MINT_EMAIL`.

Optional env overrides: `HOST_LAUNCH_MINT_CANDIDATE_ID`, `HOST_LAUNCH_MINT_EMAIL`, `HOST_LAUNCH_MINT_NAME`, `HOST_LAUNCH_MINT_JOB_COLLECTION_ID`, `HOST_LAUNCH_MINT_REQUIREMENT_ID`, `HOST_LAUNCH_MINT_TALENT_CHANNEL_ID`, `HOST_LAUNCH_MINT_CLIENT_ID`.
