...
The production session presentation uses one compact sticky context header with role, current-question text, an accessible segmented question-progress indicator, and the audience-specific safe exit. The current question remains the stable visual anchor on a soft-blue session surface while the response area moves through editable draft, save, answer acceptance, coaching wait, coaching failure, and coaching-ready states. Typed practice presents no answer-mode switcher when text is the only usable mode. Record appears only when the exact server transcription runtime and browser capture capability are available; photo and other unavailable modes remain absent rather than shown as disabled product promises. From microphone permission through recording, local-recording actions, transcription, transcript review, and answer submission, the voice interaction owns the response composer and Type/Record switching remains disabled. Retry/discard, accepted submission, or a safe terminal transcription failure releases that ownership; selecting Review does not. An accepted answer is replaced by a compact saved transcript instead of a readonly form control. Once coaching is visible, that transcript is collapsed behind an explicit review disclosure so immediate feedback receives the primary reading emphasis without hiding the candidate's own words. Staged coaching uses one un-nested coach surface, visible stage progress, persisted actions, and focus movement to each newly revealed coaching heading. Provider-internal pipeline labels must not be invented as candidate-visible progress copy.
