...
| DASH-S22 | 809 | Story | Active | Fu Chen <fu@rangam.com> | Align Practice Next with planned coverage baseline | Dashboard Practice Next should compare selected target interview evidence against the persisted question plan baseline before recommending lower-scoring matrix improvement cells, so follow-up practice stays aligned with the same stage/count plan used to generate questions |
...
| 223 | Active | DASH-S22 | 809 | Fu Chen <fu@rangam.com> | Add practice coverage baseline to Practice Next | Added `PracticeCoverageBaseline` derived from the shared `QuestionPlan`, parsed persisted `questionPlanSnapshot` in the dashboard loader, and now show completed-session coverage gaps before lower-scoring matrix improvement cells |
