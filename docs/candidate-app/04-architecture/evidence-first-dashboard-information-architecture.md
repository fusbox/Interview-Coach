...
The provider writes synthesis language only: a round title, summary, primary focus, and one comparison message per practiced question. Code owns and attaches the output status and input fingerprint, plus question order, answer/source identity, accepted coaching fields, comparison kind/count, and final artifact hydration. Provider language output must match an exact schema, preserve question order and cardinality, and pass fingerprint-envelope, score/rank, and safety-language checks before code can create candidate-safe content. Raw provider output and assembled prompts are not persisted or emitted to normal telemetry.
