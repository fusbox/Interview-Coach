...
QA/evaluation is an operator workbench for improving coach-delivered output, not another candidate dashboard or primarily a model-comparison page. It must cover candidate-led and invited answer evaluation/immediate coaching, candidate Coach Update synthesis, and generated-question quality. An operator must be able to inspect the exact candidate-visible output and accepted evidence lineage, record a structured surface-specific judgment, create reusable findings, map them to a remediation hypothesis, and recheck the same failure class after a governed change. QA workflow tables store source references and review/remediation facts rather than copied answer, JD, resume, prompt, raw-provider, or coaching payloads; authorized detail reads resolve exact immutable serving artifacts just in time. Access requires an active app user plus a separate active, manually provisioned individual AI-eval operator grant and metadata-only audit because it crosses candidate ownership. Recruiter, admin, and legacy QA roles never imply this grant. Same-input A/B execution and blind pairwise review remain compatible follow-ons, but they must not delay the core operator loop. See [AI Eval Operator Workbench](./05-quality/ai-eval-operator-workbench.md).
