...
Submitting an answer should not by itself advance to the next question. Submit saves the answer and requests coaching; the next transition should be driven by the feedback action contract, such as continue, retry/revise, finish, or pause.
...
The first completed-round bridge should keep the candidate review question-first. It may seed a sparse dashboard Coach Update, a post-round review list, and a next-practice recommendation from completed-session facts. It should lead with practiced questions, the candidate's own answer text, and coach observation/focus when available. It should distinguish skipped or unanswered questions as missing practice evidence, not as poor performance. It should not claim durable dashboard preparedness, generate a summary narrative, expose hidden scores, or reuse recruiter-oriented feedback models.
