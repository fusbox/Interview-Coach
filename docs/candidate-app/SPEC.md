...
Production V2 analysis must follow the [Evidence-First Evaluator Contract](./05-quality/evidence-first-evaluator-contract.md). One immutable answer attempt is the fixed evaluation unit. A model may extract observable, exact-span evidence and later compose coaching from code-accepted facts, but code owns schema/fingerprint validation, applicability, qualitative bands, category pattern gaps, conditional verification, safety checks, and the candidate-safe projection. Missing, unelicited, sensitive, unusable, or technically unverifiable evidence must not become a low-performance band. Technical correctness claims require a supplied versioned reference. Candidate identity, launch secrets, self-reported confidence, assembled prompts, and unvalidated raw output are excluded from normal provider/telemetry persistence. Production provider and route wiring must remain fail-closed until evaluator-run claim/retry/concurrency behavior is implemented against immutable answer-attempt lineage.
...
Session completion is the durable boundary that makes post-session synthesis eligible. The first accepted completion snapshot is immutable and duplicate completion requests must replay it rather than rewrite its timestamp or question sets. Coach Update generation runs only after completion persists and is best-effort from the completion route: a synthesis failure must not roll back the completed round or prevent dashboard return. A later replay may repair a missing artifact from the same unchanged source facts.
...
One-question `Practice this now` and fixed coach-bundle fast paths may create immutable intents directly. A coach bundle may also offer `Customize`, which seeds or merges eligible items into the durable practice builder before launch.
