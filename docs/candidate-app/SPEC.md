...
The production live-practice shell supersedes the planned/preview scaffold as candidate UI. It renders one question at a time from audience-neutral `SessionRuntimeFacts`, keeps answer and coaching mutation ownership outside the shared view, and accepts narrow completion/return adapters for candidate-led and invited practice. Candidate-led exit always returns to the dashboard context for the session's target role without clearing live progress; invited exit may later return to the invitation landing boundary. Persisted `question_preview` progress is a compatibility state only and must migrate to the equivalent live question. The candidate completion mutation, role-scoped dashboard destination, and final feedback action semantics are fixed: continue advances the round, while the last-question action completes it before dashboard return.

Invited live practice uses the same typed-answer, evidence-first evaluator, candidate-safe coaching, staged feedback, and live-shell contracts through invite-owned persistence and route adapters. The clean invited browser session must resolve the exact token-bound invited session and recipient for every mutation; candidate-led and recruiter principals do not authorize invited work. Invited attempts and evaluator runs remain normalized immutable facts with invited-session foreign keys, while invited session JSON stores only latest-state recovery projections. Minimal invited completion records the finished round and returns a simple acknowledgement; debrief and practice again remain separate contracts. The owning recruiter may open a separate read-only invited-session detail containing the immutable question set and only the latest submitted answer for each question. Drafts, superseded retries, coaching, evaluator output, timing/engagement, and candidate-led content remain outside recruiter visibility. See [Recruiter Invited Transcript Read Contract](./04-architecture/recruiter-invited-transcript-read-contract.md).
