...
Setup submission must produce a typed payload before any session-generation side effect runs. The payload includes normalized `targetRole`, normalized `jobDescription`, nullable normalized `resumeText`, `interviewStage`, `questionCount`, and resume capture mode. When candidate identity and durable dependencies are available, the server must first resolve one candidate-owned opaque prep context, create the deterministic `questionPlanSnapshot`, obtain and strictly validate the production `questionWordingSnapshot`, and only then persist the new practice session with that `roleProfileId` and immutable snapshots. It returns `/candidate/session/[sessionId]` only after durable session creation succeeds. Provider failure creates no session and does not consume trusted setup staging; the setup draft remains available for explicit retry. An explicit non-production browser bridge may still create a provisional route id when durable storage is absent, but fixture wording is restricted to explicit local host-launch development mode.
