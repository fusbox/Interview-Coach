...
- The 18-file resume suite passes: 137 tests covering policies, artifacts, parsers, OCR, route ownership, stale operations, review fencing, setup resolution, browser-draft minimization, and landing propagation.
- The full candidate suite passes: 97 files and 662 tests.
- The long-lived database readiness chain passes through migrations `001-035`, including candidate setup, concurrency, voice-source lineage, processed-resume lifecycle, selection consumption, and development seed smokes.
...
- Durable multi-instance ingestion throttling, provider/parser concurrency controls, metadata-only telemetry, and alert delivery.
