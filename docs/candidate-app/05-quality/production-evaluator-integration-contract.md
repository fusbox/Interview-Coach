...
Every case now asserts category signals plus the applicability and allowed band of all five universal criteria. The deterministic mapper also prevents `answerUsability: thin` from acquiring `clear` or `strong` criteria from isolated markers; every assessable criterion remains `observed/emerging`, while technical role skill remains `unscoreable` without a trusted reference. This is a code-owned appraisal correction, not a provider prompt or candidate-copy revision. A concise usable logistics answer remains independently protected so brevity itself is not penalized.
