...
Status: Slices 184-185 implemented; credentialed corpus calibration remains Slice 186
...
- exact synthetic answer, answer mode, and optional trusted technical reference;
...
- sensitive health, disability, family, age, immigration, compensation, conflict, or protected-class disclosure requiring a professional boundary rather than inference or legal advice;
...
- no score, rank, hiring-decision, accent, fluency, diagnosis, legal-advice, protected-trait inference, or unsupported technical-correctness language;
...
The implemented baseline contains 32 synthetic cases: the twelve evaluator golden cases, sixteen supplemental atomic cases spanning all required role/stage/resume/category dimensions, and four round journeys. Coverage validation fails if a required dimension is absent. Migration `039` adds individually granted operator drafts, exact draft-revision staging, immutable versions/suites, ownership-fenced idempotent runs, renewable targeted/global claims, terminal case/layer evidence, 30-day run retention, and metadata-only mutation audit. Operator-authored content is visible only to its owner; baseline content is shared across granted operators.
...
- `npm run db:smoke-ai-eval-scenario-workspace` applies migrations `037-040` and proves the versioned 32-case baseline, draft exact replay/conflict/revision/staging, fixture run recovery, live preview/queue/claim, accepted provider-operation checkpoint recovery without a second attempt, guarded terminalization, immediate grant revocation, 30-day retention, and content-free audit metadata. It does not call Gemini.
