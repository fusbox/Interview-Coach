...
async function main() {
    ...
    const candidateId = requireLocalInput("HOST_LAUNCH_MINT_CANDIDATE_ID");
    const email = requireLocalInput("HOST_LAUNCH_MINT_EMAIL");
...
function requireLocalInput(name: "HOST_LAUNCH_MINT_CANDIDATE_ID" | "HOST_LAUNCH_MINT_EMAIL") {
    const value = process.env[name]?.trim();
    if (!value) {
        throw new Error(`${name} is required for local host-launch token minting.`);
    }
    return value;
}
